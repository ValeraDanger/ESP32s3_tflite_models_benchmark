...
#include "int8_NoActivation_reshape_mbnetv3_minim_0.35_96x96.cc"
...
constexpr int kTensorArenaSize = 310000 + scratchBufSize;
...
void setup() {
  ...
  if (tensor_arena == NULL) {
    tensor_arena = (uint8_t *) heap_caps_malloc(kTensorArenaSize, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);  // MALLOC_CAP_INTERNAL for internal, MALLOC_CAP_SPIRAM for external psram
  }
  if (tensor_arena == NULL) {
...
void run_inference(void *ptr) {
  ...
  //printf("Prediction results: %i, %i\n", output->data.uint8[0], output->data.uint8[1]);
}
